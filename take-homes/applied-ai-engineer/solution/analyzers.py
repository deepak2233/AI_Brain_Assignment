...
class OpenAICompatibleAnalyzer:
    ...
    @classmethod
    def from_env(cls, *, model: str | None = None) -> "OpenAICompatibleAnalyzer":
        ...
        try:
            ...
            raise AnalyzerError(
                "OPENAI_MAX_ATTEMPTS and OPENAI_TIMEOUT_SECONDS must be numeric"
            ) from exc
        if not 1 <= max_attempts <= 10:
            raise AnalyzerError("OPENAI_MAX_ATTEMPTS must be between 1 and 10")
        if not 1 <= timeout_seconds <= 300:
            raise AnalyzerError("OPENAI_TIMEOUT_SECONDS must be between 1 and 300")
        return cls(
            api_key=key,
    ...
    def analyze(self, transcript: Transcript, existing_issues: list[dict[str, Any]]) -> AnalysisResult:
        ...
        try:
            for issue in result.issues:
                validate_and_ground_proposal(transcript, issue)
        except ProposalValidationError as exc:
            raise AnalyzerError(f"model proposal failed deterministic validation: {exc}") from exc
        return result
        ...
        for attempt in range(1, self.max_attempts + 1):
            try:
                ...
                if exc.code not in {408, 409, 429, 500, 502, 503, 504}:
                    raise AnalyzerError(f"model request failed ({exc.code}){suffix}") from exc
                last_error = AnalyzerError(f"model request failed ({exc.code}){suffix}")
...
class FallbackAnalyzer:
    def __init__(
        ...
        *,
        ...
        if not fallbacks:
            raise ValueError("at least one fallback analyzer is required")
        ...
        try:
            self.failure_threshold = (
                ...
                else int(os.getenv("MODEL_CIRCUIT_FAILURE_THRESHOLD", "3"))
            )
            self.cooldown_seconds = (
                cooldown_seconds
                if cooldown_seconds is not None
                else float(os.getenv("MODEL_CIRCUIT_COOLDOWN_SECONDS", "60"))
            )
        except ValueError as exc:
            raise AnalyzerError("model circuit-breaker configuration must be numeric") from exc
        if not 1 <= self.failure_threshold <= 50:
            raise AnalyzerError("MODEL_CIRCUIT_FAILURE_THRESHOLD must be between 1 and 50")
        if not 1 <= self.cooldown_seconds <= 3600:
            raise AnalyzerError("MODEL_CIRCUIT_COOLDOWN_SECONDS must be between 1 and 3600")
        self._primary_failures = 0
        self._primary_open_until = 0.0
        names = [self._identity(item) for item in (primary, *self.fallbacks)]
        self.name = "fallback[" + "->".join(names) + "]"
        self.prompt_version = primary.prompt_version

    @staticmethod
    def _identity(analyzer: Analyzer) -> str:
        model = getattr(analyzer, "model", None)
        return f"{analyzer.name}:{model}" if model else analyzer.name

    def analyze(
        self, transcript: Transcript, existing_issues: list[dict[str, Any]]
    ) -> AnalysisResult:
        failures: list[dict[str, str]] = []
        if time.monotonic() < self._primary_open_until:
            primary_identity = self._identity(self.primary)
            failures.append(
                {
                    "analyzer": primary_identity,
                    "error_type": "CircuitOpen",
                    "error": "primary analyzer temporarily bypassed",
                }
            )
            self.logger.emit(
                "analyzer_attempt_skipped",
                level="WARNING",
                call_id=transcript.call_id,
                analyzer=primary_identity,
                reason="circuit_open",
            )
            analyzers = self.fallbacks
        else:
            analyzers = (self.primary, *self.fallbacks)
        for position, analyzer in enumerate(analyzers):
            identity = self._identity(analyzer)
            is_primary = analyzer is self.primary
            try:
                result = analyzer.analyze(transcript, existing_issues)
            ...
            for issue in result.issues:
                issue.analyzer_metadata["routing"] = routing
            if fallback_used:
                self.logger.emit(
                    "analyzer_fallback_selected",
                    level="WARNING",
                    call_id=transcript.call_id,
                    analyzer=identity,
        ...
        attempted = ", ".join(item["analyzer"] for item in failures)
        raise AnalyzerError(f"all configured analyzers failed: {attempted}")
...
def _analysis_from_mapping(
    ...
    if (
        ...
        raise AnalyzerError("analysis must contain issues and ignored arrays")
    ...
    if any(not isinstance(item, dict) for item in value["ignored"]):
        raise AnalyzerError("every ignored signal must be an object")
    ignored = [
...
def analyzer_from_name(
    ...
    *,
    ...
    config = config or RuntimeConfig.from_env(exercise_root)
    normalized = name.lower()
    if normalized == "heuristic":
        config.validate_provider(normalized)
        return HeuristicAnalyzer()
    if normalized not in {"auto", "openai"}:
        raise AnalyzerError(f"unknown provider {name!r}; choose auto, openai, or heuristic")

    config.validate_provider(normalized)
    if normalized == "auto" and not os.getenv("OPENAI_API_KEY"):
        return HeuristicAnalyzer()

    primary = OpenAICompatibleAnalyzer.from_env()
    fallbacks: list[Analyzer] = []
    fallback_model = os.getenv("OPENAI_FALLBACK_MODEL", "").strip()
    if fallback_model and fallback_model != primary.model:
        fallbacks.append(OpenAICompatibleAnalyzer.from_env(model=fallback_model))
    if config.allow_heuristic_fallback:
        fallbacks.append(HeuristicAnalyzer())
    return FallbackAnalyzer(primary, fallbacks, logger) if fallbacks else primary
