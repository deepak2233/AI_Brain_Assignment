...
class EventLogger:
    ...
    def emit(self, event: str, **fields: Any) -> None:
        if not self.enabled:
            return
        value = {"event": event, "timestamp": _utc_timestamp(), **fields}
        print(json.dumps(value, ensure_ascii=False, sort_keys=True), file=self.stream, flush=True)
...
class IntakePipeline:
    ...
    def scan(self, paths: Iterable[str | Path]) -> RunSummary:
        ordered = sorted({Path(path).resolve() for path in paths}, key=lambda item: item.name)
        ...
        processed = skipped = failed = proposals_created = sources_attached = suppressed_shipped = 0
        for path in ordered:
            ...
            try:
                ...
                self.store.record_failure(run_id, call_id, transcript_sha, duration_ms, repr(exc))
                self.logger.emit(
                    "transcript_failed",
                    run_id=run_id,
                    call_id=call_id,
        ...
        self.store.finish_run(run_id, processed=processed, skipped=skipped, failed=failed)
        summary = RunSummary(
            run_id=run_id,
            requested=len(ordered),
            processed=processed,
        ...
        self.logger.emit("run_completed", **summary.to_dict())
        return summary
