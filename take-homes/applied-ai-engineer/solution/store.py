...
class IntakeStore:
    def __init__(self, path: str | Path):
        ...
        self.conn.execute("PRAGMA secure_delete = FAST")
        self._initialize()
        violations = self.conn.execute("PRAGMA foreign_key_check").fetchall()
        if violations:
            self.conn.close()
            raise StoreError("database foreign-key integrity check failed")
    ...
    def _initialize(self) -> None:
        self.conn.executescript(
            ...
            CREATE TABLE IF NOT EXISTS runs (
                ...
                model_proposals INTEGER NOT NULL DEFAULT 0,
            ...
            CREATE TABLE IF NOT EXISTS outbox (
                ...
                status TEXT NOT NULL CHECK (status IN ('pending', 'sending', 'delivered', 'error', 'dead')),
        ...
        self._ensure_run_metrics()
        self._ensure_dead_letter_status()

    def _ensure_run_metrics(self) -> None:
        columns = {
            str(row["name"])
            for row in self.conn.execute("PRAGMA table_info(runs)").fetchall()
        }
        if "model_proposals" not in columns:
            self.conn.execute(
                "ALTER TABLE runs ADD COLUMN model_proposals INTEGER NOT NULL DEFAULT 0"
            )
        if "invalid_proposals" not in columns:
            self.conn.execute(
                "ALTER TABLE runs ADD COLUMN invalid_proposals INTEGER NOT NULL DEFAULT 0"
            )

    def _ensure_dead_letter_status(self) -> None:
        row = self.conn.execute(
            "SELECT sql FROM sqlite_master WHERE type='table' AND name='outbox'"
        ).fetchone()
        if row and "'dead'" in str(row["sql"]):
            return
            ...
            self.conn.executescript(
                ...
                CREATE TABLE outbox (
                    ...
                    status TEXT NOT NULL CHECK (
                        status IN ('pending', 'sending', 'delivered', 'error', 'dead')
                    ),
                    attempts INTEGER NOT NULL DEFAULT 0,
                    last_error TEXT,
                    result TEXT,
                ...
                INSERT INTO outbox(
                    event_key, review_id, sink, payload, depends_on, status, attempts,
                    last_error, result, created_at, updated_at
                )
                SELECT event_key, review_id, sink, payload, depends_on, status, attempts,
                       last_error, result, created_at, updated_at
                FROM outbox_legacy;
    ...
    def finish_run(
        ...
        *,
        ...
        with self.transaction():
            self.conn.execute(
                "UPDATE runs SET finished_at=?, status=?, processed=?, skipped=?, failed=?, "
                "model_proposals=?, invalid_proposals=? WHERE id=?",
                (
                    utc_now(),
                    status,
                    processed,
    ...
    def record_success(
        ...
        *,
        ...
        self.conn.execute(
            ...
            "VALUES (?, ?, ?, 'processed', ?, ?) "
            "ON CONFLICT(run_id, call_id, transcript_sha) DO UPDATE SET "
            "status='processed', duration_ms=excluded.duration_ms, error=NULL, analysis=excluded.analysis",
    ...
    def record_skip(self, run_id: str, call_id: str, transcript_sha: str) -> None:
        with self.transaction():
            self.conn.execute(
                ...
                "VALUES (?, ?, ?, 'skipped') "
                "ON CONFLICT(run_id, call_id, transcript_sha) DO NOTHING",
    ...
    def mark_outbox_error(self, event_key: str, error: str, max_attempts: int = 5) -> str:
        with self.transaction():
            row = self.conn.execute(
                "SELECT attempts FROM outbox WHERE event_key=?", (event_key,)
            ).fetchone()
            if not row:
                raise StoreError(f"unknown outbox event: {event_key}")
            status = "dead" if int(row["attempts"]) >= max_attempts else "error"
            self.conn.execute(
                "UPDATE outbox SET status=?, last_error=?, updated_at=? WHERE event_key=?",
                (status, error[:4000], utc_now(), event_key),
            )
        return status
    ...
    def recover_interrupted_outbox(self, max_attempts: int = 5) -> int:
        with self.transaction():
            cursor = self.conn.execute(
                "UPDATE outbox SET status=CASE WHEN attempts >= ? THEN 'dead' ELSE 'error' END, "
                "last_error='recovered interrupted delivery', updated_at=? "
                "WHERE status='sending'",
                (max_attempts, utc_now()),
            )
        return cursor.rowcount

    def retry_dead(self, event_key: str | None = None) -> int:
        with self.transaction():
            if event_key:
                cursor = self.conn.execute(
                    "UPDATE outbox SET status='error', attempts=0, last_error=NULL, updated_at=? "
                    "WHERE status='dead' AND event_key=?",
                    (utc_now(), event_key),
                )
            else:
                cursor = self.conn.execute(
        ...
        if event_key and cursor.rowcount == 0:
            raise StoreError(f"no dead-letter event matches {event_key!r}")
        return cursor.rowcount

    def get_outbox(self, event_key: str) -> dict[str, Any] | None:
        row = self.conn.execute(
            "SELECT * FROM outbox WHERE event_key=?", (event_key,)
        ).fetchone()
        return self._decode(row) if row else None

    def list_dead_letters(self) -> list[dict[str, Any]]:
        rows = self.conn.execute(
            "SELECT event_key, review_id, sink, depends_on, status, attempts, "
            "last_error, created_at, updated_at FROM outbox WHERE status='dead' "
    ...
    def health(self) -> dict[str, Any]:
        ...
        return {
            "status": (
                "ok"
                if quick_check == "ok" and foreign_key_violations == 0
                else "unhealthy"
            ),
    ...
    def counts(self) -> dict[str, int]:
        queries = {
            ...
            "model_proposals": "SELECT COALESCE(SUM(model_proposals), 0) FROM runs",
            "invalid_proposals": "SELECT COALESCE(SUM(invalid_proposals), 0) FROM runs",
            ...
            "outbox_dead": "SELECT COUNT(*) FROM outbox WHERE status='dead'",
            "outbox_blocked": (
                "SELECT COUNT(*) FROM outbox o JOIN outbox d ON d.event_key=o.depends_on "
                "WHERE o.status='pending' AND d.status='dead'"
            ),
        }
