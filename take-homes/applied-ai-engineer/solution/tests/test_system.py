...
class ParserAndSafetyTests(unittest.TestCase):
    ...
    def test_repeated_quote_requires_source_disambiguation(self) -> None:
        with tempfile.TemporaryDirectory(prefix="betterbark-repeat-quote-") as temporary:
            path = Path(temporary) / "call-999.md"
            quote = "The same exact customer sentence appears twice in this call."
            path.write_text(
                "# Call — Example Co × BetterBark · Review\n"
                "Date: 2026-01-01 · Call ID: call-999\n"
                f"[EXTERNAL] First Customer: {quote}\n"
                f"[EXTERNAL] Second Customer: {quote}\n"
                "[INTERNAL] Owner: Thanks.\n",
                encoding="utf-8",
            )
            transcript = parse_transcript(path)
            grounded = ground_evidence(
                transcript,
                [Evidence(quote, "First Customer", 3, 3)],
            )
            self.assertEqual(grounded[0].line_start, 3)
            with self.assertRaises(ProposalValidationError):
                ground_evidence(transcript, [Evidence(quote, "unknown", 99, 99)])
