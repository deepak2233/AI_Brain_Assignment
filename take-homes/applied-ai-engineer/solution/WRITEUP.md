...
durable outbox, then the dispatcher calls the supplied stubs. Reviewers see the
...
structured model call per transcript. A conservative heuristic baseline keeps
the repository runnable without credentials and provides a deterministic smoke
test. I would not use that baseline alone for the unlabeled production stream.
...
event and receives the returned Jira key. In production I would use Jira/Slack
request identifiers where supported plus a reconciliation worker; I would not
claim exactly-once against an API that cannot provide it.
...
Every run emits JSON events for start, per-call success/skip/failure, rejected
ungrounded proposals, dedupe outcome, sink attempt, and aggregate completion.
The SQLite `status` view exposes processed, failed, pending, rejected, and
undelivered counts. In production those become counters and alerts for zero-
output runs, error-rate spikes, review backlog age, action-rate drift, duplicate-
rate drift, and dead-lettered outbox events.
...
I deliberately omitted a web UI, embedding service, concurrent workers, real
credentials, and automatic filing. They add surface area without proving the
safety loop. With another day I would run and calibrate the model path across
repeated trials, add a model adjudicator only for ambiguous duplicate pairs,
implement real Jira comment/update and Slack Block Kit adapters, add PII
retention/redaction policy, and export metrics/traces to the production
observability stack. Approximate focused build and verification time: three
hours, AI-assisted.
