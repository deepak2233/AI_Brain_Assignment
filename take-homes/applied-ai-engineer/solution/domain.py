...
def parse_transcript(path: str | Path, *, max_bytes: int | None = None) -> Transcript:
    path = Path(path)
    with path.open("rb") as handle:
        raw_bytes = handle.read(max_bytes + 1) if max_bytes is not None else handle.read()
    if max_bytes is not None and len(raw_bytes) > max_bytes:
        raise TranscriptParseError(f"{path}: transcript exceeds {max_bytes} byte input limit")
    try:
        raw = raw_bytes.decode("utf-8")
    except UnicodeDecodeError as exc:
        raise TranscriptParseError(f"{path}: transcript is not valid UTF-8") from exc
...
def _select_evidence_turn(candidates: list[Turn], item: Evidence) -> Turn | None:
    if len(candidates) == 1:
        return candidates[0]
    by_line = [turn for turn in candidates if turn.line_no == item.line_start]
    if len(by_line) == 1:
        return by_line[0]
    speaker = item.speaker.strip().casefold()
    by_speaker = [turn for turn in candidates if turn.speaker.casefold() == speaker]
    return by_speaker[0] if len(by_speaker) == 1 else None
...
def ground_evidence(transcript: Transcript, evidence: Iterable[Evidence]) -> list[Evidence]:
    ...
    external_by_text: dict[str, list[Turn]] = {}
    for external_turn in transcript.external_turns:
        external_by_text.setdefault(_normalized_exact(external_turn.text), []).append(
            external_turn
        )
    for item in evidence:
        quote = _normalized_exact(item.quote)
        turn = _select_evidence_turn(external_by_text.get(quote, []), item)
        ...
        if turn is None and len(item.quote.strip()) >= 20:
            ...
            turn = _select_evidence_turn(substring_matches, item)
            if turn is not None:
                grounded_quote = item.quote.strip()
        if turn is None:
            raise ProposalValidationError(
                f"evidence is not an exact EXTERNAL utterance in {transcript.call_id}"
            )
        grounded.append(
            Evidence(
...
def validate_and_ground_proposal(transcript: Transcript, proposal: ProposedIssue) -> ProposedIssue:
    ...
    if not proposal.summary.strip() or len(proposal.summary) > 180:
        raise ProposalValidationError("summary must be 1-180 characters")
    if not proposal.product_area.strip() or len(proposal.product_area) > 120:
        raise ProposalValidationError("product area must be 1-120 characters")
    if not proposal.description.strip() or len(proposal.description) > 12000:
        raise ProposalValidationError("description must be 1-12000 characters")
    if not proposal.rationale.strip() or len(proposal.rationale) > 4000:
        raise ProposalValidationError("rationale must be 1-4000 characters")
    if proposal.severity not in {"S1", "S2", "S3", "S4"}:
        raise ProposalValidationError(f"unsupported severity: {proposal.severity!r}")
    if not 0.0 <= proposal.confidence <= 1.0:
        raise ProposalValidationError("confidence must be between 0 and 1")
    if len(proposal.evidence) > 10:
        raise ProposalValidationError("proposal cannot contain more than 10 evidence quotes")
